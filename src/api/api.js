...
const apiPostCall = (endpoint) => (additionalHeaders = {}) => async (body) => {
...
const createBuddy = (jwt) => (userId, name, notes, birthday, cadence, location) => {
...
const archiveBuddy = (jwt) => (userId, buddyId) => authApiPostCall(jwt)("buddy/archive")({
	user_id: userId,
	buddy_id: buddyId,
});

const archiveInteractions = (jwt) => (userId, interactionId) => authApiPostCall(jwt)("interaction/archive")({
	user_id: userId,
...
const setupFakeData = async (fakeUserEmail, fakeUserPassword) => {
	...
	const fakeBuddies = [
		...
		[userId, "Brendon Daugherty", "Brendon (also known as Ken) plays bass", "10-02-1997"],
		[userId, "Porter Sherman", "Porter is the lead guitarist of 'The Band'", "07-21-1997"],
		[userId, "Charlie Coburn", "Charlie is a noodly boi", "07-01-1997"],
		[userId, "Hannah Brenchley", "Hannah is an absolute pal, and knows the best muzak like parcels", "12-23-1997"],
		[userId, "Miela Mayer", "Miela is a hectic human being that we all know and love", "07-17-1996"],
	];

	const fakeInteractions = [
		// userId, notes, participants, Option<date>
		[userId, "Told me that they are quitting their job", [0], "10-02-2021"],
		[userId, "Told me they are stoked for their sister's wedding", [2], "07-03-2020"],
	...
	for (const fakeBuddy of fakeBuddies) {
		...
		let resp = await buddiesService.createBuddy(jwt)(userId, name, notes, birthday);
		console.log(resp);
	...
	for (const fakeInteraction of fakeInteractions) {
		...
		await buddiesService.createInteraction(jwt)(userId, notes, participants, date);
	}
